...
| Network               | Drop `proxy` network from base compose entirely (caddy is off-host on both envs). Base compose binds `${HOST_BIND_IP:?...}:3000:3000` — `HOST_BIND_IP` is **required** (compose fails fast if unset). Staging `.env` sets `HOST_BIND_IP=10.0.88.3`; prod `.env` must be updated to `HOST_BIND_IP=10.0.88.2` before the next prod tag is cut. No overrides, no stub networks. After prod deploy, `rm ~/email-to-telegram/docker-compose.override.yml` on prod. |
...
1. **Rewrite `docker-compose.yml`** (one file, prod + staging both use it):
   - Remove the top-level `proxy` external network entirely (caddy is off-host on both envs).
   - Remove `proxy` from app's `networks:` list (only `internal` remains).
   - Add `ports: ["${HOST_BIND_IP:?...}:3000:3000"]` to app. `HOST_BIND_IP` is required — compose refuses to start if unset. Per-env set in `.env`.
2. **Rewrite `.github/workflows/deploy-staging.yml`**:
   - `runs-on: [self-hosted, linux, x64, email-tg-staging]`.
   - Steps mirror prod: `scp docker-compose.yml staging:~/email-to-telegram/` then `ssh staging bash <<EOF` for `docker login ghcr` → `IMAGE_TAG=main docker compose --env-file .env pull/up` → 30×5s health-poll.
   - Drop `SSH_HOST_STAGING / SSH_USER_STAGING / SSH_PORT_STAGING / SSH_KEY_STAGING` secret references.
3. Add `HOST_BIND_IP=` placeholder + comment to `.env.example`.
4. Update `README.md` to describe the new single-compose / `HOST_BIND_IP`-driven shape.
5. Open PR — **do not merge yet**.
...
2. `scp staging.env vprojects:~/email-to-telegram/.env && ssh vprojects 'chmod 600 ~/email-to-telegram/.env'`. Static keys baked in: `MAIL_DOMAIN`, `HOSTED_MAIL_DOMAIN`, `PUBLIC_BASE_URL`, `HTTP_PORT=3000`, `HOST_BIND_IP=10.0.88.3`, `APP_MODE=hosted`, `BILLING_PROVIDER=none`, `ADMIN_ENABLED=true`, `STORAGE_ENCRYPTION_MODE=local-v1`, `MASTER_ENCRYPTION_KEY_ID=staging-v1`, `METRICS_ENABLED=true`, `BACKUP_DIR=/data/backups`, `BACKUP_ARCHIVE_ENCRYPTION=storage-key`, `INITIAL_ALLOWED_USERS=419515180`, `ALERT_CHAT_ID=419515180`, `LOG_LEVEL=info`, `NODE_ENV=production`. Plus placeholder `TELEGRAM_BOT_TOKEN=__SET_BEFORE_LAUNCH__`.
3. No manual `docker login` — workflow handles it on every deploy.
4. **Firewall check**: with `HOST_BIND_IP=10.0.88.3` the published port is only on the private interface, so the public internet cannot reach it. Within the private LAN it's reachable by anyone with a 10.0.88.x address — fine for the shared Caddy VM, acceptable given current trust posture. If multi-tenant LAN ever grows, add nftables rule scoping port 3000 ingress to the Caddy VM's IP only.
...
- [ ] `.env` on vprojects exists with 600 perms and has `HOST_BIND_IP=10.0.88.3`.
- [ ] Prod `.env` is updated to set `HOST_BIND_IP=10.0.88.2` before the next prod tag is cut (otherwise `docker compose up` on prod will fail fast — by design — with "HOST_BIND_IP must be set").
...
2. `ssh vprojects 'cd ~/email-to-telegram && IMAGE_TAG=main docker compose --env-file .env up -d --no-deps app'` (only restart app; leave postgres alone).
...
- `proxy` external network removed from base compose entirely (no stub needed, no override file).
- Port bound to the private interface via required `HOST_BIND_IP`; compose fails fast if unset, preventing accidental `0.0.0.0` binds in any environment.
